---
slug: /references/medusa-workflows/createOrderCreditLinesWorkflow
sidebar_label: createOrderCreditLinesWorkflow
sidebar_description: ""
tags:
  - order
  - workflow
...
This documentation provides a reference to the `createOrderCreditLinesWorkflow`. It belongs to the `@medusajs/medusa/core-flows` package.

<SourceCodeLink link="https://github.com/alse-sym/medusa/blob/84ffdab81b1701b2db2ae3ff1df5f42d861308f0/packages/core/core-flows/src/order/workflows/create-order-credit-lines.ts#L86" />
...
export async function POST(
  ...
  const { result } = await createOrderCreditLinesWorkflow(req.scope)
    .run({
      input: {
        "id": "id_pQOYFGzAeXtfNRtn",
        "credit_lines": [
          "{value}"
        ]
      }
...
export default async function handleOrderPlaced({
  ...
  const { result } = await createOrderCreditLinesWorkflow(container)
    .run({
      input: {
        "id": "id_pQOYFGzAeXtfNRtn",
        "credit_lines": [
...
const myWorkflow = createWorkflow(
  "my-workflow",
  () => {
    const result = createOrderCreditLinesWorkflow
      .runAsStep({
        input: {
          "id": "id_pQOYFGzAeXtfNRtn",
          "credit_lines": [
...
createOrderCreditLinesWorkflow.hooks.creditLinesCreated(
  (async ({}, { container }) => {
    //TODO
  })
)
...
<TypeList types={[{"name":"input","type":"`object`","description":"The input data for the hook.","optional":false,"defaultValue":"","expandable":false,"children":[]}]} expandUrl="https://docs.medusajs.com/learn/fundamentals/data-models/manage-relationships#retrieve-records-of-relation" openedLevel={1} sectionTitle="creditLinesCreated"/>
