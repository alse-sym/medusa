...
sidebar_description: Update one or more product collections.
tags:
  - product
  - event bus
  - workflow
...
[Create Collection Admin API Route](https://docs.medusajs.com/api/admin#collections\_postcollectionsid).
...
<SourceCodeLink link="https://github.com/alse-sym/medusa/blob/84ffdab81b1701b2db2ae3ff1df5f42d861308f0/packages/core/core-flows/src/product/workflows/update-collections.ts#L59" />
...
export async function POST(
  ...
  const { result } = await updateCollectionsWorkflow(req.scope)
    .run({
      input: {
        selector: {
          id: "pcol_123"
...
const myWorkflow = createWorkflow(
  "my-workflow",
  () => {
    const result = updateCollectionsWorkflow
      .runAsStep({
        input: {
          selector: {
            id: "pcol_123"
